...
class AppPackageSoftwareImageInfo(BaseModel):
    ...
    version: constr(regex=r'^\d+(?:\.\d+){0,2}$') = Field(
        ..., description='Version of the software image.', example=0.1
    )
    checksum: Checksum
...
class AppPkgInfo(BaseModel):
    ...
    appSoftwareVersion: Optional[constr(regex=r'^\d+(?:\.\d+){0,2}$')] = Field(
        None,
    ...
    appDescriptorVersion: Optional[constr(regex=r'^\d+(?:\.\d+){0,2}$')] = Field(
        None,
...
class ExperimentDf(BaseModel):
    ...
    nsInstantiationLevel: List[ExperimentLevel] = Field(
        ..., description='Describes the details of an Experiment level.', min_items=1
    )
...
class AppPkgEvent(BaseModel):
    ...
    Payload: Optional[Payload] = Field(None, description='Event payload')
...
class AdditionalServiceData(BaseModel):
    portData: List[ServicePortData] = Field(
        ..., description='Service port numbers exposed by the CP.', min_items=1
    )
